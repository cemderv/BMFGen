...
static inline QColor colorFromTheme(Qt::ColorScheme newScheme)
{
    return newScheme == Qt::ColorScheme::Dark ? QColor{210, 210, 210} : QColor{120, 120, 120};
}
    ...
    , _disableParentWhenSpinning(disableParentWhenSpinning)
{
    ...
    setColor(colorFromTheme(qApp->styleHints()->colorScheme()));
    ...
    connect(qApp->styleHints(),
            ...
            [this](Qt::ColorScheme newScheme) {
                Q_UNUSED(newScheme);
                setColor(colorFromTheme(newScheme));
            });
}
    ...
    int countDistance, int totalNrOfLines, qreal trailFadePerc, qreal minOpacity, QColor color)
{
    ...
    else
    {
        ...
        color.setAlphaF(std::clamp(resultAlpha, 0.0, 1.0));
    }
